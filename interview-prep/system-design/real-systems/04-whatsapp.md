[🏠 Home](../../../README.md) | [⬅️ 03 Uber](./03-uber.md) | [➡️ 05 Netflix Interview](./05-netflix/01-system-design-interview.md)

# 💬 WhatsApp System Design

> Design a messaging system for 2 billion users

---

## ✅ Functional Requirements

| Feature | Description |
|---------|-------------|
| **1:1 Chat** | Send/receive messages |
| **Group Chat** | Up to 256 members |
| **Media Sharing** | Images, videos, documents |
| **Read Receipts** | Delivered, seen status |
| **Online Status** | Last seen, typing indicator |

---

## 📊 Scale Estimation

- **Users**: 2 billion
- **DAU**: 500 million
- **Messages/day**: 100 billion
- **Message size**: ~100 bytes average

---

## 🏗️ High-Level Architecture

```
┌─────────────────────────────────────────────────────────────────────┐
│                           CLIENTS                                   │
│    ┌──────────┐                              ┌──────────┐          │
│    │  User A  │                              │  User B  │          │
│    │  (App)   │                              │  (App)   │          │
│    └────┬─────┘                              └─────┬────┘          │
└─────────┼──────────────────────────────────────────┼────────────────┘
          │                                          │
          ▼                                          ▼
┌─────────────────────────────────────────────────────────────────────┐
│                       CONNECTION LAYER                              │
│    ┌────────────────┐                  ┌────────────────┐          │
│    │  WebSocket     │                  │  WebSocket     │          │
│    │  Server 1      │                  │  Server 2      │          │
│    └───────┬────────┘                  └────────┬───────┘          │
└────────────┼────────────────────────────────────┼───────────────────┘
             │                                    │
             ▼                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│                         SERVICES                                    │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌─────────┐ │
│  │   Message    │  │   Presence   │  │    Group     │  │  Media  │ │
│  │   Service    │  │   Service    │  │   Service    │  │ Service │ │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └────┬────┘ │
└─────────┼─────────────────┼─────────────────┼───────────────┼──────┘
          │                 │                 │               │
          ▼                 ▼                 ▼               ▼
┌─────────────────────────────────────────────────────────────────────┐
│                         DATA LAYER                                  │
│    ┌──────────────┐   ┌──────────────┐   ┌──────────────┐          │
│    │  Cassandra   │   │    Redis     │   │      S3      │          │
│    │  (Messages)  │   │  (Sessions)  │   │   (Media)    │          │
│    └──────────────┘   └──────────────┘   └──────────────┘          │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 📨 Message Flow

### Online User (Real-time Delivery)

```
User A                WebSocket 1       Message Service      WebSocket 2       User B
  │                       │                   │                   │               │
  │──── Send message ────▶│                   │                   │               │
  │                       │── Store & Route ─▶│                   │               │
  │                       │                   │── Store in DB ───▶│               │
  │                       │                   │                   │               │
  │                       │                   │── Forward ───────▶│               │
  │                       │                   │                   │──Deliver──▶   │
  │                       │                   │                   │               │
  │                       │                   │                   │◀── ACK ───────│
  │                       │                   │◀─ Update status ──│               │
  │                       │◀── Delivered ────│                   │               │
  │◀──── ✓✓ Delivered ────│                   │                   │               │
  │                       │                   │                   │               │
```

### Offline User (Store & Forward)

```
User A              Message Service         Database           User B (Offline)
  │                       │                     │                     │
  │─── Send message ─────▶│                     │                     │
  │                       │─── Store msg ──────▶│                     │
  │                       │                     │     [OFFLINE]       │
  │◀───── ✓ Sent ─────────│                     │                     │
  │                       │                     │                     │
  │                       │                     │     [COMES ONLINE]  │
  │                       │◀──── Sync request ──────────────────────────│
  │                       │─── Get pending ────▶│                     │
  │                       │◀── [msg1,msg2...] ──│                     │
  │                       │──────── Deliver ───────────────────────────▶│
  │                       │◀──────── ACK all ──────────────────────────│
  │◀──── ✓✓ Delivered ────│                     │                     │
```

---

## 💾 Data Storage

### Message Schema (Cassandra)
```sql
CREATE TABLE messages (
    chat_id UUID,
    message_id TIMEUUID,
    sender_id UUID,
    content BLOB,  -- Encrypted
    media_url TEXT,
    status TEXT,   -- sent, delivered, read
    created_at TIMESTAMP,
    PRIMARY KEY (chat_id, message_id)
) WITH CLUSTERING ORDER BY (message_id DESC);
```

### Why Cassandra?
- Write-heavy workload (100B messages/day)
- Horizontal scaling
- Time-series data (messages ordered by time)
- High availability

### Connection Mapping (Redis)
```
user:{user_id}:connection → ws_server_1
user:{user_id}:status → "online" | "last_seen:timestamp"
```

---

## 🔐 End-to-End Encryption

```
┌─────────────────────┐                      ┌─────────────────────┐
│       USER A        │                      │       USER B        │
│  ┌───────────────┐  │                      │  ┌───────────────┐  │
│  │ Private Key A │  │                      │  │ Private Key B │  │
│  └───────┬───────┘  │                      │  └───────┬───────┘  │
│  ┌───────┴───────┐  │                      │  ┌───────┴───────┐  │
│  │ Public Key A  │──┼──────────────────────┼─▶│ Public Key B  │  │
│  └───────────────┘  │                      │  └───────────────┘  │
└─────────────────────┘                      └─────────────────────┘
          │                                            │
          │  1. A encrypts with B's public key         │
          │                                            │
          ▼                                            │
   ┌─────────────┐                                     │
   │  Encrypted  │─────────────────────────────────────▶
   │   Message   │     [Server cannot read this]       │
   └─────────────┘                                     │
                                                       ▼
                                           2. B decrypts with
                                              B's private key
                                                       │
                                                       ▼
                                               ┌─────────────┐
                                               │ Plain Text  │
                                               │   Message   │
                                               └─────────────┘
```

**Signal Protocol:**
- Double Ratchet Algorithm
- Pre-keys for offline messaging
- Server cannot read messages

---

## 👥 Group Messaging

```
                        ┌───────────────────────┐
                        │       SENDER          │
                        └───────────┬───────────┘
                                    │
                                    ▼
                        ┌───────────────────────┐
                        │    Message Service    │
                        └───────────┬───────────┘
                                    │
                     ┌──────────────┼──────────────┐
                     │              │              │
                     ▼              ▼              ▼
              ┌──────────┐  ┌──────────┐   ┌──────────┐
              │ Member 1 │  │ Member 2 │   │ Member N │
              └──────────┘  └──────────┘   └──────────┘
                     
                     Fan-out: Send to ALL members
```

### Group Metadata
```json
{
  "group_id": "uuid",
  "name": "Family Group",
  "members": ["user1", "user2", "user3"],
  "admins": ["user1"],
  "created_at": "2026-01-01"
}
```

---

## 📱 Presence & Typing Indicators

### Last Seen

```
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│ User opens   │─────▶│  WebSocket   │─────▶│    Redis     │
│   app        │      │   Server     │      │              │
└──────────────┘      └──────────────┘      └──────────────┘
                                                    │
                                                    ▼
                                            user:123:last_seen
                                            = "2026-01-26 16:30"
```

### Typing Indicator

```
User A                 WebSocket                User B
  │                       │                        │
  │─── typing:start ─────▶│                        │
  │                       │── "A is typing..." ───▶│
  │                       │                        │ [Shows indicator]
  │─── typing:stop ──────▶│                        │
  │      (or send msg)    │── Clear indicator ────▶│
  │                       │                        │ [Hides indicator]
```

**Optimization:** Throttle typing events (send every 2 seconds max)

---

## ⚡ Key Optimizations

| Optimization | Purpose |
|-------------|---------|
| **Persistent Connections** | Reduce connection overhead |
| **Message Compression** | Reduce bandwidth |
| **Local DB (SQLite)** | Offline access, fast loading |
| **Media CDN** | Fast media delivery |
| **Message Batching** | Reduce network calls |

---

## 🌍 Real-World Examples

### Example 1: Sending a Photo

```
┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
│ User A   │     │  Media   │     │   CDN    │     │   User B │
│ (Sender) │     │ Service  │     │   (S3)   │     │(Receiver)│
└────┬─────┘     └────┬─────┘     └────┬─────┘     └────┬─────┘
     │                │                │                │
     │── Upload ─────▶│                │                │
     │   photo        │── Store ──────▶│                │
     │                │◀── URL ────────│                │
     │◀─ Media URL ───│                │                │
     │                │                │                │
     │──── Send message with URL ──────────────────────▶│
     │                │                │                │
     │                │                │◀── Download ───│
     │                │                │─── Photo ─────▶│
```

**Real Use Case:** 
- Sending wedding photos to family group
- Photo uploaded once to S3
- All 50 group members download from CDN (not sender's device)

---

### Example 2: Business WhatsApp - Order Notification

```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│   Zomato     │     │  WhatsApp    │     │   Customer   │
│   Backend    │     │ Business API │     │    Phone     │
└──────┬───────┘     └──────┬───────┘     └──────┬───────┘
       │                    │                    │
       │── Order placed ───▶│                    │
       │   template msg     │                    │
       │                    │── Send message ───▶│
       │                    │                    │ "Order #1234 
       │                    │                    │  confirmed!"
       │                    │◀──── Delivered ────│
       │◀── Webhook: ───────│                    │
       │    delivered       │                    │
```

**Real Use Case:**
- Swiggy/Zomato order confirmation
- Flipkart delivery updates  
- Bank OTP messages
- Uber ride confirmations

---

### Example 3: Group Video Call Setup

```
┌─────────┐  ┌─────────┐  ┌─────────┐     ┌─────────────┐
│ User A  │  │ User B  │  │ User C  │     │ Signaling   │
│(Caller) │  │         │  │         │     │   Server    │
└────┬────┘  └────┬────┘  └────┬────┘     └──────┬──────┘
     │            │            │                  │
     │── Start call ──────────────────────────────▶│
     │            │            │                  │
     │            │◀──── Incoming call ───────────│
     │            │            │◀── Incoming call ─│
     │            │            │                  │
     │            │── Accept ─────────────────────▶│
     │            │            │── Accept ────────▶│
     │            │            │                  │
     │◀────────── P2P Connection Setup ───────────▶│
     │            │            │                  │
     │◀═══════════╪════════════╪══════════════════╡
     │     Direct video/audio stream (WebRTC)     │
     │════════════╪════════════╪══════════════════▶│
```

**Real Use Case:**
- Family group calling during festivals
- Team standup calls
- After initial setup, data flows P2P (not through servers)

---

### Example 4: Message Status Updates

```
Status      Symbol     Meaning
─────────────────────────────────────
Sending     ⏳         Message in queue
Sent        ✓          Reached server
Delivered   ✓✓         Reached recipient's device
Read        ✓✓ (blue)  Recipient opened chat


Timeline:
┌─────────────────────────────────────────────────────────────┐
│  0ms        50ms       200ms        2s          5s          │
│   │          │           │          │           │           │
│   ▼          ▼           ▼          ▼           ▼           │
│  ⏳─────────▶✓──────────▶✓✓────────▶✓✓(blue)                │
│ Sending    Sent      Delivered    Read                      │
└─────────────────────────────────────────────────────────────┘
```

---

## 📚 Key Takeaways

- WebSocket for real-time bidirectional communication
- Cassandra for high-write message storage
- Redis for connection mapping and presence
- End-to-end encryption for privacy
- Fan-out for group messages

---

*WhatsApp handles 100B+ messages daily with this architecture.*
