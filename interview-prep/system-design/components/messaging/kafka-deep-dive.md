...
Kafka architecture is built around partitioned append-only logs, pull-based consumption, leader-follower replication with ISR, OS-level page cache optimization, and horizontal scaling via partitions across brokers. It achieves durability through replicated logs and high throughput through sequential disk I/O and batching.


Log compaction is used when the topic represents the latest state of an entity rather than a full event history. The key typically represents a unique entity identifier such as a database primary key, orderId, userId, or configuration name.

In real-world systems, the key in a compacted topic typically represents the unique identifier of an entity, such as a database primary key, orderId, or configuration key. Kafka keeps only the latest record per key, making it ideal for CDC, state storage, and distributed configuration systems.
